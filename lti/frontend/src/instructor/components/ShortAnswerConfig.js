...
const ShortAnswerConfig = ({ items, setItems }) => {
    ...
    useEffect(() => {
        console.log('shortAnswerItems updated:', items);
      }, [items]);
  
    return (
      <section className='p-4 w-full'>
        <section className='flex items-center'>
        <form onSubmit={handleAddItem} className="mb-4 w-full mr-4 flex items-center gap-2">
            <Input
            ...
            />
            <div className="w-fit">
                <Button label="Add" className="px-4 py-2 text-sm" />
            </div>
        </form>
