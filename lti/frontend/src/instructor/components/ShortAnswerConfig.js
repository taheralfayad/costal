...
const ShortAnswerConfig = ({ items, setItems, isMath }) => {
        ...
        <ul className='flex flex-col gap-2 mb-4'>
          {items.map((item, index) => (
            <li key={index} className='flex items-center justify-between p-2 border rounded-md bg-white shadow'>
              <p className='text-sm font-medium text-gray-700'>{isMath ? <span dangerouslySetInnerHTML={{ __html: item }} /> : item}</p>
