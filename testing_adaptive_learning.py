...
test_questions['difficulty'] = test_questions.apply(lambda _: random.randint(1, 3), axis=1)


# Fit the BKT model for the module
if not os.path.exists(f'./adaptive-engine-api/models/bkt_model_{COURSE_ID}_{MODULE_ID}.pkl'):
...
if not os.path.exists(f'./adaptive-engine-api/models/mab_model_{COURSE_ID}_{ASSIGNMENT_ID}.pkl'):
...
while attempts_per_skill['22'][1] < 0.95 or attempts_per_skill['27'][1] < 0.95 or attempts_per_skill['32'][1] < 0.95:
    ...
    response = requests.post(f'{ADAPTIVE_ENGINE_URL}/run-model-on-response/', json=request)
