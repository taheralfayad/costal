...
class LLMEngine:
    ...
    def generate_questions(
        ...
        try:
            ...
            user_message_1 = (
                f"From the following topic or objective, extract the most relevant concept "
                "to base a multiple choice question on:\n\n"
                f"{topic}\n\n"
            ...
            try:
                ...
                raise ValueError(
                    "Concept extraction failed or did not return a valid JSON string.",
                    concept_response,
                )
            ...
            avoidance_clause = (
                f"Critically, the new question MUST be substantially different from "
                f"these previous questions:\n{prev_q_texts}\n\n"
                if prev_q_texts
                else ""
            )
            ...
            user_message_2 = (
                f"Generate a **completely new and unique** multiple-choice question "
                f"focusing on the core idea of this concept: '{concept}'.\n\n"
                f"{avoidance_clause}"
                "Ensure the question tests understanding, not just recall. "
                "Phrase it differently using no markup language than any "
                "previous examples provided.\n"
                "The question must have exactly 4 plausible answer choices "
                "in no markup language labeled A, B, C, and D. "
                "You may use numbers and equations. "
                "One choice must be unambiguously correct.\n"
                "Return ONLY a valid JSON object with NO markup language "
                "with keys 'question' (string) and 'options' "
                "(object with keys 'A', 'B', 'C', 'D'), like:\n"
                "{\n"
            )
            ...
            try:
                ...
                raise ValueError(
                    "Question drafting failed or did not return valid JSON."
                )

            system_prompt_3 = (
                "You are an expert grader. You will be shown a "
            ...
            try:
                ...
                raise ValueError(
                    "Answer identification failed or returned invalid option."
                )
