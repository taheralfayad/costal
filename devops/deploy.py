...
subprocess.run(
    ["docker", "compose", "-f", "docker-compose.deploy.yml", "build"], check=True
)
...
subprocess.run(
    [
        "docker",
        "tag",
        "costal-adaptive-engine",
        f"backend:adaptive-engine-{COMMIT_TAG}",
    ],
    check=True,
)
...
login = subprocess.Popen(
    ["aws", "ecr", "get-login-password", "--region", AWS_REGION], stdout=subprocess.PIPE
)
...
subprocess.run(
    [
        "docker",
        "tag",
        f"backend:web-{COMMIT_TAG}",
        f"{ECR_REPOSITORY}:web-{COMMIT_TAG}",
    ],
    check=True,
)
subprocess.run(
    [
        ...
        "tag",
        f"backend:adaptive-engine-{COMMIT_TAG}",
        f"{ECR_REPOSITORY}:adaptive-engine-{COMMIT_TAG}",
    ],
    check=True,
)
subprocess.run(["docker", "push", f"{ECR_REPOSITORY}:web-{COMMIT_TAG}"], check=True)
subprocess.run(
    ["docker", "push", f"{ECR_REPOSITORY}:adaptive-engine-{COMMIT_TAG}"], check=True
)
...
autoscaling.start_instance_refresh(AutoScalingGroupName="costal-asg")
